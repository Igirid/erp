...
const specificSort = ref<keyof typeof props.payments | null>(null);
...
//   return [...props.payments].sort((a: any, b: any) => {
//     const key = specificSort.value as keyof typeof props.payments;
...
const openModal = (id?: number) => {
  if (id) {
    // selectedPayment.value = sortedPayments.value[id];
    selectedPayment.value = props.payments[id];
...
const handleSelection = (action: string) => {
  ...
  if (action == "delete") {
    openModal(selectedPayment.value?.id);
  } else if (action == "view") {
    router.push(`/payments/${selectedPayment.value?.id}`);
  }
};
...
const selectPayment = (id: number, paymentId: number) => {
...
<template>
  ...
  >
    <table class="w-full">
      <thead>
        <tr class="bg-gray-100 text-center text-xs font-semibold text-gray-600">
          ...
          <th
            ...
            @click="updateSpecificSort('paymentName')"
          >
            Payment Name <ChevronsUpDown class="inline w-[12px]" />
          </th>
          <th
            class="p-3 font-semibold"
      ...
      <tbody class="text-xs font-medium">
        ...
        >
          <td class="p-3">
            <NuxtLink :to="`payments/${payment.id}`">
              {{ payment.docNo }}
            </NuxtLink>
          </td>
          <td class="p-3">
          ...
          <!-- v-if="id" -->
          <PaymentsDropdownMenu
            v-if="selectedPayment?.id == payment.id"
  ...
  <!-- <div v-else class="grid sm:grid-cols-2 lg:grid-cols-4 gap-[30px] h-auto">
    <div v-for="payment in props.payments" :key="payment.id">
      ...
      >
        <p class="font-semibold">Name: {{ payment.paymentName }}</p>
        <p>Contact: {{ payment.contactName }}</p>
        <p>Phone: {{ payment.phone }}</p>
        <p
          :class="['mt-2 px-2 max-w-[200px] w-[40%] text-center py-1 rounded-full text-xs', statuses[payment.status as keyof typeof statuses]]"
        >
          {{ payment.status }}
        </p>
      </div>
